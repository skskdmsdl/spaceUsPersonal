				...
				<div class="col-lg-12">
					<div class="property-submit-form">
						...
						<div class="pf-feature-price" style="margin-bottom: 20px;">
							<h4>공간 호스트 이메일</h4>
							<div class="mx-lg-5 member">
								<h5>${space.memberEmail}</h5>
							</div>
						</div>
								...
								<div class="col-md-8 mx-auto">
									<div id="myCarousel" class="carousel slide"
										...
										<ol class="carousel-indicators">
											<c:forEach items="${sapceOneImageList}" var="image"
												varStatus="vs">
										...
										<div class="carousel-inner">
											<c:forEach items="${sapceOneImageList}" var="image"
												...
												<div class="carousel-item" id="spaceImg">
													<img
														src="${pageContext.request.contextPath}/resources/upload/space/${image.renamedFilename}"
														class="img-fluid" alt="">
												</div>
											</c:forEach>
										</div>
						...
						<div class="pf-summernote" style="margin-bottom: 20px;">
							<h4>공간설명</h4>
							<div class="mx-lg-5">${space.content}</div>
						</div>
							...
							<div class=" mx-lg-5">
								<c:forEach items="${spaceOneCategory}" var="category">
									<span class="badge badge-warning badge-pill"
										style="font-size: 15px;">${category.categoryName}</span>
								</c:forEach>
							</div>
						</div>
								...
								<c:if test="${!empty spaceOneTagList}">
									<c:forEach items="${spaceOneTagList}" var="tag">
										<span class="badge badge-success badge-pill"
											style="font-size: 15px;">${tag.tagName }</span>
									</c:forEach>
								</c:if>
								<c:if test="${empty spaceOneTagList}">
								...
								<c:if test="${!empty spaceOneOptionList}">
									<c:forEach items="${spaceOneOptionList}" var="option">
										<span class="badge badge-primary badge-pill"
											style="font-size: 15px;">${option.optionName}</span>
									</c:forEach>
								</c:if>
								<c:if test="${empty spaceOneOptionList}">
						...
						<form>
							<div class="pf-property-details" style="text-align: center;">
								...
								<button type="button" class="confirm btn-info col-sm-3"
									value="${space.spaceNo}">공간 승인</button>
							</div>
						</form>
...
$(".confirm").click(function(){
	...
	var param1 = "spaceNo="+spaceNo+"&memberEmail="+memberEmail;

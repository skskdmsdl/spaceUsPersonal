...
<div class="page-wrapper">
	<div class="container-fluid">
		...
		<div id="talkjs-container"
			style="background-color: rgba(0, 0, 0, 0); margin-left: 300px; height: 500px; position: fixed; z-index: 10; box-shadow: rgba(0, 0, 0, 0.5) 0 0 0 9999px; display: none;">
			<i>Loading chat...</i>
		</div>
								...
								<c:forEach items="${ wlist }" var="wish" varStatus="vs">
									<div class="col-md-3" style="width: 90%; display: inline;">
										<div class="thumbnail block-18">
											<a
												href="${pageContext.request.contextPath }/space/spaceDetail.do?spaceNo=${wish.spaceNo}">
												<img class="block-20"
												src="${ pageContext.request.contextPath }/resources/upload/space/${wish.image}"
												alt="...">
											</a>
										</div>
										<div class="caption">
											<div class="space-detail">
												...
												<span class="badge badge-light"> <a
													href="${pageContext.request.contextPath }/space/spaceDetail.do?spaceNo=${wish.spaceNo}">
														<c:set var="str1" value="${wish.spaceName }" /> ${ fn:substring(str1, 0, 12) }
												</a>
												</span> <span style="font-size: 13px;"><i
													class="icon-my_location badge badge-secondary">${ wish.address}</i></span>

												<ul class="space_list">
													<li><i class="icon-won badge badge-secondary"><fmt:formatNumber
																value="${wish.hourlyPrice }" type="number" /><small>원/시간</small></i>
													<li>
													<li><i class="icon-star"></i> <c:if
															test="${wish.starAvg != 0 }">
															<fmt:formatNumber value="${wish.starAvg}" pattern="0.0"
																type="number" />
														</c:if> <c:if test="${wish.starAvg == 0 }">0</c:if></li>
											...
											<div class="content">
												<a
													href="${pageContext.request.contextPath }/space/spaceDetail.do?spaceNo=${wish.spaceNo}">
													<p class="content" style="width: 250px;">
														<c:set var="str2" value="${wish.content }" />${ fn:substring(str2, 0, 120) }</p>
												</a>
											</div>
											<form name="deleteFrm"
												...
												<p style="text-align: center; width: 75%;">
													<button class="delete btn btn-secondary" type="submit">위시리스트
														삭제</button>
												</p>
											</form>
										</div>
